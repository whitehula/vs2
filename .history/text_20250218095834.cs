    ...
    class Program
    {
        ...
        public class enemyMelee:enemy
        {
            ...
            public sealed override void Move()
            {
                ...
                /*密封：
                当一个方法或属性被声明为 sealed 时，它不能在其子类中被重写（override）。
                sealed 只能用于已经重写了基类方法的方法或属性（即它必须与 override 一起使用）。*/
        }}
        ...
        public class Unit
        {
            ...
            //方法的重载：要可以区分（参数数量不同、访问权限不同），不能只改返回类型
            public int add(int a)
            {
                return a;
            }
            private bool add(int a,int b)
            {
                return a + b > 0;
            }
        }
